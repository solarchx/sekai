            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6">
                    <h3 class="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-6">
                        {{ __('Create New Activity Form') }}</h3>
                    ...
                    <form method="POST" action="{{ route('activity-forms.store') }}">
                        ...
                        <div class="mb-6">
                            ...
                            <select name="activity_id" id="activity_id"
                                ...
                                @foreach($activities as $activity)
                                    <option value="{{ $activity->id }}" {{ old('activity_id') == $activity->id ? 'selected' : '' }}>{{ $activity->subject->name }} - {{ $activity->teacher->name }}
                        ...
                        <div class="mb-6">
                            <label for="activity_date"
                                class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Activity Date') }}</label>
