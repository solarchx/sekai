    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6">
                    <h3 class="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-6">Edit Activity Form</h3>
                    ...
                    <form method="POST" action="{{ route('activity-forms.update', $activityForm) }}">
                        ...
                        <div class="mb-6">
                            ...
                            <select name="activity_id" id="activity_id" class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white @error('activity_id') is-invalid @enderror" required>
                                ...
                                @foreach($activities as $activity)
                                    <option value="{{ $activity->id }}" {{ old('activity_id', $activityForm->activity_id) == $activity->id ? 'selected' : '' }}>{{ $activity->subject->name }} - {{ $activity->teacher->name }} ({{ $activity->class->name }})</option>
                                @endforeach
                            </select>
                            @error('activity_id')
                        ...
                        <div class="mb-6">
                            ...
                            <input type="date" name="activity_date" id="activity_date" value="{{ old('activity_date', $activityForm->activity_date) }}" class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white @error('activity_date') is-invalid @enderror" required>
                            @error('activity_date')
                                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
