    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6">
                    <h3 class="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-6">{{ __('Edit Activity Form') }}
                    </h3>

                    <form method="POST" action="{{ route('activity-forms.update', $activityForm) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-6">
                            <label for="activity_date"
                                class="block text-sm font-medium text-gray-700 dark:text-gray-300">Activity Date</label>
                            <input type="date" name="activity_date" id="activity_date"
                                value="{{ old('activity_date', $activityForm->activity_date) }}"
                                class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white @error('activity_date') is-invalid @enderror"
                                required>
                            @error('activity_date')
                                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                            @enderror
