        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6" style="background: linear-gradient(to right, #8b5cf6, #a855f7); color: white;">
                    ...
                    <p class="mt-2">Select an activity to view its forms.</p>
                </div>
                <div class="p-6">
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full bg-white dark:bg-gray-800">
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                @forelse($forms as $form)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 {{ $form->deleted_at ? 'bg-red-50 dark:bg-red-900' : '' }}">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                            {{ $form->activity->subject->name ?? 'N/A' }} - {{ $form->activity->class->name ?? 'N/A' }}
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                            @if($form->deleted_at)
                                                <form action="{{ route('activity-forms.restore', $form) }}" method="POST" class="inline">
                                                    @csrf
                                                    <button type="submit" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg shadow-md transition-colors" title="Restore">
                                                        <i class="bi bi-arrow-counterclockwise"></i> Restore
                                                    </button>
                                                </form>
                                            @else
                                                <a href="{{ route('activity-forms.show', $form) }}" class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded-lg text-xs inline-block">View</a>
                                                <a href="{{ route('activity-forms.edit', $form) }}" class="bg-violet-600 hover:bg-violet-700 text-white px-3 py-1 rounded-lg text-xs inline-block">Edit</a>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">No forms found</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
