            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6">
                    <h3 class="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-6">
                        {{ __('Edit Presence Record') }}</h3>

                    <form method="POST"
                        action="{{ route('activity-presences.update', ['activityForm' => $form, 'activityPresence' => $activityPresence]) }}"
                        id="presenceForm">
                        @csrf
                        @method('PUT')

                        <input type="hidden" name="location" id="location" value="{{ $activityPresence->location }}">
                        ...
                        <div class="mb-6">
                            ...
                            <div class="space-y-2">
                                <div class="flex items-center">
                                    <input type="radio" name="score" id="score_0" value="0" {{ old('score', $activityPresence->score) == '0' ? 'checked' : '' }}
                                ...
                                <div class="flex items-center">
                                    <input type="radio" name="score" id="score_1" value="1" {{ old('score', $activityPresence->score) == '1' ? 'checked' : '' }}
                                ...
                                <div class="flex items-center">
                                    <input type="radio" name="score" id="score_2" value="2" {{ old('score', $activityPresence->score) == '2' ? 'checked' : '' }}
                                        class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300">
                                    <label for="score_2"
                                ...
                                <div class="flex items-center">
                                    <input type="radio" name="score" id="score_3" value="3" {{ old('score', $activityPresence->score) == '3' ? 'checked' : '' }}
                                        class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300">
                                    <label for="score_3"
                        ...
                        <div class="flex justify-end gap-4">
                            <a href="{{ route('activity-presences.index', ['form_id' => $form->id]) }}"
            ...
            if (!locationInput.value) {
                if (navigator.geolocation) {
                    navigator.geolocation.getCurrentPosition(
                        ...
                        function (error) {
                            alert('Unable to retrieve your location. The existing location will be kept.');
                        }
                    );
                } else {
