    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                {{ isset($form) ? __('Attendance for ') . $form->activity->subject->name . ' - ' . $form->activity_date : __('Activity Presence Management') }}
            </h2>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6" style="background: linear-gradient(to right, #3b82f6, #1d4ed8); color: white;">
                    <h3 class="text-2xl font-bold">{{ isset($form) ? 'Attendance for ' . $form->activity->subject->name : 'Activity Presence Management' }}</h3>
                    <p class="mt-2">{{ isset($form) ? 'Date: ' . $form->activity_date : 'Manage student presence records.' }}</p>
                </div>
                <div class="p-6">
                                    ...
                                    @forelse($students as $student)
                                        ...
                                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                                ...
                                                @else
                                                    <a href="{{ route('activity-presences.create', ['form_id' => $form->id, 'student_id' => $student->id]) }}" class="bg-green-600 hover:bg-green-700 text-white px-3 py-1 rounded-lg text-xs inline-block">Record</a>
                                                @endif
                                            </td>
                                        </tr>
                                    @empty
                    ...
                    @else
                        <div class="flex justify-between items-center mb-6">
                            <h4 class="text-xl font-semibold text-gray-900 dark:text-gray-100">Presences List ({{ $presences->total() }})</h4>
                        </div>
                        <div class="overflow-x-auto">
                            <table class="min-w-full bg-white dark:bg-gray-800">
                                <thead class="bg-gray-50 dark:bg-gray-700">
                        ...
                        <div class="mt-6 flex justify-between items-center">
                            <div class="text-sm text-gray-600 dark:text-gray-400">
                                Showing {{ $presences->firstItem() }} to {{ $presences->lastItem() }} of {{ $presences->total() }} results
                            </div>
                    ...
                    @if(isset($form))
                        <div class="mt-6">
                            <a href="{{ route('activity-forms.show', $form) }}" class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg shadow-md transition-colors">
                                Back to Form
                            </a>
                        </div>
                    @endif
