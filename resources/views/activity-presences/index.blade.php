        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6" style="background: linear-gradient(to right, #3b82f6, #1d4ed8); color: white;">
                    <h3 class="text-2xl font-bold">{{ __('Activity Presence Management') }}</h3>
                    <p class="mt-2">{{ __('Select an activity and a form to view or manage attendance.') }}</p>
                </div>
                <div class="p-6">
                    ...
                    @if($selectedForm)
                        <div class="mt-6">
                            <h4 class="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-4">
                                {{ __('Attendance for :date', ['date' => $selectedForm->activity_date->format('M d, Y')]) }}
                            </h4>
                                        ...
                                        @forelse($students as $student)
                                            ...
                                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                                    @if($presence)
                                                        <a href="{{ route('activity-presences.edit', ['activityForm' => $selectedForm, 'activityPresence' => $presence]) }}"
                                                            class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded-lg text-xs inline-block">{{ __('Edit') }}</a>
                                                    @else
                                                        <a href="{{ route('activity-presences.create', ['form_id' => $selectedForm->id, 'student_id' => $student->id]) }}"
                                                            class="bg-green-600 hover:bg-green-700 text-white px-3 py-1 rounded-lg text-xs inline-block">{{ __('Record') }}</a>
                                                    @endif
                                                </td>
                                            </tr>
                                        @empty
                                            <tr>
                                                <td colspan="4" class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">
                                                    {{ __('No students enrolled in this class.') }}</td>
                                            </tr>
                                        @endforelse
                                    </tbody>
                                </table>
                    ...
                    @elseif($selectedActivity)
                        <div
                            ...
                            {{ __('Please select a form from the dropdown above.') }}
                        </div>
                    ...
                    @else
                        <div
                            class="bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg p-4 text-gray-600 dark:text-gray-300 mt-6">
                            {{ __('Select an activity from the dropdown above.') }}
                        </div>
                    @endif
                </div>
            </div>
