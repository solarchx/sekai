                        ...
                        <table class="min-w-full bg-white dark:bg-gray-800">
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                @forelse($presences as $presence)
                                    <tr
                                        class="hover:bg-gray-50 dark:hover:bg-gray-700 {{ $presence->deleted_at ? 'bg-red-50 dark:bg-red-900' : '' }}">
                                        <td
                                            class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-gray-100">
                                            {{ $presence->id }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                            {{ $presence->student->name ?? 'N/A' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                            Form #{{ $presence->form_id }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                            {{ $presence->score }}</td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                            @if($presence->deleted_at)
                                                <form action="{{ route('activity-presences.restore', $presence) }}"
                                            ...
                                            @else
                                                <button
                                                    onclick="window.location.href='{{ route('activity-presences.edit', $presence) }}'"
                                                    class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg shadow-md transition-colors"
                                                    title="Edit">Edit</button>
                                                <form action="{{ route('activity-presences.destroy', $presence) }}"
                                                    method="POST" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
