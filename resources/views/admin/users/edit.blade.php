        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6" style="background: linear-gradient(to right, #6366f1, #3b82f6); color: white;">
                    <h3 class="text-2xl font-bold">{{ __('Edit User') }}</h3>
                    <p class="mt-2">{{ __('Edit the user account with the required details.') }}</p>
                </div>
                <div class="p-6">
                    <form method="POST" action="{{ route('users.update', $user->id) }}">
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            ...
                            <div>
                                ...
                                <select name="role" id="role" required
                                    ...
                                    <option value="TEACHER" {{ old('role', $user->role) == 'TEACHER' ? 'selected' : '' }}>
                                        {{ __('Teacher') }}</option>
                                    <option value="VP" {{ old('role', $user->role) == 'VP' ? 'selected' : '' }}>
                                        {{ __('VP') }}</option>
                            ...
                            @if ($user->role === 'STUDENT')
                                <div>
                                    <label for="class_id"
                                        class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('Class') }}</label>
                            ...
                            @if($user->role === 'STUDENT')
                                <div id="activity-section" style="{{ $user->class_id ? '' : 'display: none;' }}">
                                    <label for="activity_ids"
                                        ...
                                        {{ __('Enrolled Activities') }}
                                    </label>
                                    <select name="activity_ids[]" id="activity_ids" multiple
                                        class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white"
                                        size="6">
                                        @foreach($classActivities as $activity)
                                            <option value="{{ $activity->id }}" {{ in_array($activity->id, old('activity_ids', $enrolledActivityIds)) ? 'selected' : '' }}>
                                                {{ $activity->subject->name }} – {{ $activity->period->weekday_name }}
                                                {{ $activity->period->time_begin }}-{{ $activity->period->time_end }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                        {{ __('Hold Ctrl/Cmd to select multiple.') }}</p>
                        ...
                        <div class="flex items-center justify-end mt-6">
                            <a href="{{ route('users.index') }}"
                                class="mr-4 text-gray-600 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white">{{ __('Cancel') }}</a>
                            <button type="submit"
