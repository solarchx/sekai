                    ...
                    <form method="POST" action="{{ route('periods.update', $period) }}">
                        ...
                        <div class="mb-6">
                            ...
                            <select name="weekday" id="weekday" class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white @error('weekday') is-invalid @enderror" required>
                                ...
                                <option value="0" {{ old('weekday', $period->weekday) == '0' ? 'selected' : '' }}>Sunday</option>
                                <option value="1" {{ old('weekday', $period->weekday) == '1' ? 'selected' : '' }}>Monday</option>
                                <option value="2" {{ old('weekday', $period->weekday) == '2' ? 'selected' : '' }}>Tuesday</option>
                                <option value="3" {{ old('weekday', $period->weekday) == '3' ? 'selected' : '' }}>Wednesday</option>
                                <option value="4" {{ old('weekday', $period->weekday) == '4' ? 'selected' : '' }}>Thursday</option>
                                <option value="5" {{ old('weekday', $period->weekday) == '5' ? 'selected' : '' }}>Friday</option>
                                <option value="6" {{ old('weekday', $period->weekday) == '6' ? 'selected' : '' }}>Saturday</option>
                            </select>
                            @error('weekday')
                        ...
                        <div class="mb-6">
                            ...
                            <input type="time" name="time_begin" id="time_begin" value="{{ old('time_begin', $period->time_begin) }}" class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white @error('time_begin') is-invalid @enderror" required>
                            @error('time_begin')
                                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <select name="semester_id" id="semester_id" class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white @error('semester_id') is-invalid @enderror" required>
                                ...
                                @foreach($semesters as $semester)
                                    <option value="{{ $semester->id }}" {{ old('semester_id', $period->semester_id) == $semester->id ? 'selected' : '' }}>{{ $semester->full_name }}</option>
                                @endforeach
                            </select>
                            @error('semester_id')
                                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
