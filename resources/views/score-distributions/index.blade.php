            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                ...
                <div class="p-6">
                    ...
                    <div class="flex justify-between items-center mb-6">
                        <h4 class="text-xl font-semibold text-gray-900 dark:text-gray-100">Distributions List
                            ({{ $distributions->total() }})</h4>
                        @if(!$showDeleted)
                            <a href="{{ route('score-distributions.create') }}"
                        ...
                        <table class="min-w-full bg-white dark:bg-gray-800">
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                @forelse($distributions as $distribution)
                                    <tr
                                        class="hover:bg-gray-50 dark:hover:bg-gray-700 {{ $distribution->deleted_at ? 'bg-red-50 dark:bg-red-900' : '' }}">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                            {{ $distribution->activity->subject->name }} -
                                            {{ $distribution->activity->class->name }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                            @if($distribution->deleted_at)
                                                <form action="{{ route('score-distributions.restore', $distribution) }}"
                                            ...
                                            @else
                                                <button
                                                    onclick="window.location.href='{{ route('score-distributions.edit', [$distribution->activity_id, $distribution->name]) }}'"
                                                    class="bg-amber-600 hover:bg-amber-700 text-white px-4 py-2 rounded-lg shadow-md transition-colors"
                                                    title="Edit">Edit</button>
                                                <form
                                                    action="{{ route('score-distributions.destroy', [$distribution->activity_id, $distribution->name]) }}"
                                                    method="POST" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
