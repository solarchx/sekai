        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6" style="background: linear-gradient(to right, #6366f1, #3b82f6); color: white;">
                    <h3 class="text-2xl font-bold">{{ __('Activity Forms') }}</h3>
                    <p class="mt-2">{{ __('Select an activity to view its forms and submit your attendance.') }}</p>
                </div>
                <div class="p-6">
                    {{-- Activity selector --}}
                    <form method="GET" action="{{ route('student.activities') }}" id="activity-form">
                        <div class="mb-6 flex items-end gap-4">
                            ...
                            <noscript>
                                <button type="submit"
                                    class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg">{{ __('Go') }}</button>
                            </noscript>
                        </div>
                    </form>
                    ...
                    @if($selectedActivity)
                        <div class="mt-8">
                            <h4 class="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-4">
                                {{ __('Forms for :name', ['name' => $selectedActivity->subject->name]) }}
                            </h4>

                            @if($forms->isEmpty())
                                <p class="text-gray-500 dark:text-gray-400">{{ __('No forms have been created for this activity yet.') }}
                                </p>
                            @else
                                                ...
                                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                                    ...
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                                        @if($presence)
                                                            ...
                                                            @endphp
                                                            <span
                                                                class="bg-{{ $statusColors[$presence->score] }}-100 dark:bg-{{ $statusColors[$presence->score] }}-900 text-{{ $statusColors[$presence->score] }}-800 dark:text-{{ $statusColors[$presence->score] }}-200 px-3 py-1 rounded-full text-xs font-semibold">
                                                                {{ $statusLabels[$presence->score] }}
                                                            </span>
                                                        @else
                                                            <span class="text-gray-400">{{ __('Not submitted') }}</span>
                                                        @endif
                                                    </td>
                                                        ...
                                                        @if($presence)
                                                            @if($report)
                                                                <a href="{{ route('activity-reports.edit', $report) }}"
                                                                    class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded-lg text-xs">{{ __('Edit Report') }}</a>
                                                                <form action="{{ route('activity-reports.destroy', $report) }}"
                                                                    method="POST" class="inline"
                                                                    onsubmit="return confirm('{{ __('Are you sure?') }}');">
                                                                    @csrf
                                                                    @method('DELETE')
                                                                    <button type="submit"
                                                                        class="bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded-lg text-xs">{{ __('Delete Report') }}</button>
                                                                </form>
                                                            @else
                                                                <a href="{{ route('activity-reports.create', ['presence_id' => $presence->id]) }}"
                                                                    class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded-lg text-xs">{{ __('Create Report') }}</a>
                                                            @endif
                                                        @else
                                                            @if($canSubmit)
                                                                <a href="{{ route('activity-presences.create', ['form_id' => $form->id, 'student_id' => auth()->id()]) }}"
                                                                    class="bg-green-600 hover:bg-green-700 text-white px-3 py-1 rounded-lg text-xs">{{ __('Submit Presence') }}</a>
                                                            @else
                                                                <span class="text-gray-400">{{ __('Submission closed') }}</span>
                                                            @endif
                                                        @endif
                                                    </td>
                                                </tr>
                    ...
                    @elseif($activities->isNotEmpty())
                        <div
                            ...
                            {{ __('Please select an activity from the dropdown above.') }}
                        </div>
                    @else
                        <div
                            ...
                            {{ __('You are not enrolled in any activities.') }}
                        </div>
                    @endif
                </div>
            </div>
